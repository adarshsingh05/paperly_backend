...
router.get("/", authenticateToken, async (req, res) => {
  try {
    ...
    const employeeDetails = await EmployeeDetail.find({ ownerEmail: userEmail });
...
router.put("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (!updatedEmployee) {
      return res.status(404).json({ 
        success: false,
        error: "Employee details not found or you don't have permission to update it." 
      });
    }
...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (!deletedEmployee) {
      return res.status(404).json({ 
        success: false,
        error: "Employee details not found or you don't have permission to delete it." 
      });
    }
